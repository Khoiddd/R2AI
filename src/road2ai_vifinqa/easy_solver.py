...
You must return ONLY this exact JSON object. No thinking. No explanation. No other text.

{"selected_ids":["e000306"],"operation":"value","confidence":0.95,"reason":"short reason"}

Rules:
- selected_ids must be real IDs from the candidate list (never invent e000001)
- operation: "value" | "sum" | "difference" | "abs_difference"
- Most questions need exactly one id + "value"
...
def _preview(
    candidates: list[EasyCandidate], *, max_serialized_chars: int = 12_000
